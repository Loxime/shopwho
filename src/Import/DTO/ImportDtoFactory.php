...
final class ImportDtoFactory
{
    ...
    {
        return match ($type) {
            'users' => new UserImportDto($record, $this->text($row,'externalRef'), $this->text($row,'email'), $this->nullableText($row,'firstName'), $this->nullableText($row,'lastName'), $this->date($row,'createdAt')),
            'products' => new ProductImportDto($record, $this->text($row,'externalRef'), $this->text($row,'name'), $this->text($row,'slug'), $this->text($row,'description'), $this->integer($row,'priceCents'), $this->integer($row,'stock'), $this->text($row,'categorySlug'), $this->nullableText($row,'imageUrl'), $this->boolean($row,'isActive')),
            'orders' => new OrderImportDto($record, $this->text($row,'externalRef'), $this->text($row,'userExternalRef'), $this->text($row,'status'), $this->date($row,'orderedAt'), $this->nullableInteger($row,'totalCents')),
            'order_items' => new OrderItemImportDto($record, $this->text($row,'orderExternalRef'), $this->nullableText($row,'productExternalRef'), $this->text($row,'productNameSnapshot'), $this->text($row,'productSlugSnapshot'), $this->integer($row,'quantity'), $this->integer($row,'unitPriceCents')),
            'reviews' => new ReviewImportDto($record, $this->text($row,'externalRef'), $this->text($row,'userExternalRef'), $this->text($row,'productExternalRef'), $this->integer($row,'rating'), $this->nullableText($row,'comment'), $this->date($row,'createdAt')),
            default => throw new ImportException(sprintf('Unsupported import type "%s".', $type)),
        };
    }
